...
Put thidevtools::install_github("bdilday/GeomMLBStadiums")s in the console:
...
```{r}
ggplot(DJLeMahieu, aes(x = release_pos_x, y = release_pos_z)) + # 2D density plot
  geom_density2d()
```

```{r}
ggplot(DJLeMahieu, aes(x = pitch_type, y = release_speed)) +
  geom_violin()

```

```{r}
ggplot(DJLeMahieu, aes(x = pitch_type, y = events)) +
  geom_count()
```

```{r}
ggplot(DJLeMahieu, aes(x = events, y = release_speed, fill = description)) +
  geom_boxplot() +
...
ggplot(DJLeMahieu, aes(x = hc_x_, y = hc_y_)) +
  ...
  coord_fixed() +
  theme_void()
# spraychart is unique to package
```
