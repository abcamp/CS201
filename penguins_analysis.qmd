...
author: "Connor Flynn (edited by Amber Camp)"
format: html
editor: visual
---
...
ggplot(data = penguins_data_from_csv, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  ...
  scale_color_viridis_d(option="inferno") +
   labs(title = "Palmer Penguins, Culmen Depth & Body Mass",
       subtitle = "3 penguin species observed on three islands in the Palmer Archipelago, Antarctica",
       x = "culmen depth (mm)",
       y = "body mass (g)")
## that light yellow on white is no bueno
```
...
average_species_flipper_length <- penguins_data_from_csv %>% 
  group_by(species) %>% 
  summarise(average_flipper_length_mm = mean(flipper_length_mm, na.rm = TRUE), average_flipper_length_sd = sd(flipper_length_mm, na.rm = TRUE)) # added sd calculation here so that I can use it below
view(average_species_flipper_length)
```
...
```{r, message=FALSE}
ggplot(data = average_species_flipper_length, aes(x = species, y = average_flipper_length_mm, fill = species)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin=average_flipper_length_mm-average_flipper_length_sd, ymax=average_flipper_length_mm+average_flipper_length_sd), width=0.25, colour="orange", alpha=0.75, linewidth=0.5) + # just for fun. here is where i used sd so i can get +/- error bars
  theme_linedraw() +
  scale_x_discrete(labels=c('Adelie', 'Chinstrap', 'Gentoo')) +
  xlab("Penguin Species") +
  ylab("Average Flipper Length (mm)") +
  labs(title = "Average Flipper Length by Species (Palmer Penguins)", fill="Species") +
  scale_fill_viridis_d(option="mako") #+
  #theme(legend.position = "none") #if you wanted to remove the legend, but I don't right now
...
summary(gentoo_adelie_penguins_data_joined) # inspect
unique(gentoo_adelie_penguins_data_joined$Species) # confirm
gentoo_adelie_penguins_data_joined %>% # count
  count(Species)
```
...
```{r, message=FALSE}
boxplot_species_body_mass <- ggplot(data = penguins_data_from_csv, aes(x = species, y = body_mass_g , fill = species)) +
  geom_boxplot() +
  theme_bw() +
  scale_x_discrete(labels=c('Adelie', 'Chinstrap', 'Gentoo')) +
  xlab("Penguin Species") +
  ylab("Body Mass (g)") +
  labs(title = "Body Mass by Species (Palmer Penguins)", fill="Species") +
  scale_fill_viridis_d(option="magma")

ggplotly(boxplot_species_body_mass)
```
...
# Use summary
penguins_data_from_csv$island <- as.factor(penguins_data_from_csv$island)
summary(penguins_data_from_csv$island)


# You can use dplyr, and you wouldn't even have to re-class the variable first:
penguins_data_from_csv %>%
  count(island)
```

1.  Create a dataframe that has the average culmen depth for each species
...
average_species_culmen_depth <- penguins_data_from_csv %>% 
  group_by(species) %>% 
  summarise(average_culmen_depth_mm = mean(culmen_depth_mm, na.rm = TRUE), average_culmen_depth_sd = sd(culmen_depth_mm, na.rm = TRUE))
#view(average_species_culmen_depth)

plot_average_species_culmen_depth <- ggplot(data = average_species_culmen_depth, aes(x = species, y = average_culmen_depth_mm, fill = species)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin=average_culmen_depth_mm-average_culmen_depth_sd, ymax=average_culmen_depth_mm+average_culmen_depth_sd), width=0.25, colour="orange", alpha=0.75, linewidth=0.5) + # just for fun
  theme_bw() +
  scale_x_discrete(labels=c('Adelie', 'Chinstrap', 'Gentoo')) +
  xlab("Penguin Species") +
  ylab("Average Culmen Depth (mm)") +
  labs(title = "Average Culmen Depth by Species (Palmer Penguins)", fill="Species") +
  scale_fill_viridis_d() 

ggplotly(plot_average_species_culmen_depth)

## Hey Connor! I wonder why when you hover over any of the bars in this plot, 
## "species" is listed twice. Any ideas?
```
