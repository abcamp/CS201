...
average_species_flipper_length <- penguins_data_from_csv %>% 
  group_by(species) %>% 
  summarise(average_flipper_length_mm = mean(flipper_length_mm, na.rm = TRUE), average_flipper_length_sd = sd(flipper_length_mm, na.rm = TRUE))
...
ggplot(data = average_species_flipper_length, aes(x = species, y = average_flipper_length_mm, fill = species)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin=average_flipper_length_mm-average_flipper_length_sd, ymax=average_flipper_length_mm+average_flipper_length_sd), width=0.25, colour="orange", alpha=0.75, linewidth=0.5) + # just for fun
  theme_bw() +
...
```{r, message=FALSE}
plot_average_species_flipper_length <- ggplot(data = average_species_flipper_length, aes(x = species, y = average_flipper_length_mm, fill = species)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin=average_flipper_length_mm-average_flipper_length_sd, ymax=average_flipper_length_mm+average_flipper_length_sd), width=0.25, colour="orange", alpha=0.75, linewidth=0.5) +
  theme_bw() +
  scale_x_discrete(labels=c('Adelie', 'Chinstrap', 'Gentoo')) +
  xlab("Penguin Species") +
  ylab("Average Flipper Length (mm)") +
...
ggplotly(plot_average_species_flipper_length)

## Hey Connor! I wonder why when you hover over any of the bars below, "species" is listed twice. Any ideas?
```
...
#Answer is 52. Here is another nice way to answer this, and you dont even have to re-class the variable first:
